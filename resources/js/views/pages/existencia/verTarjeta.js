            ...
            this.axios
                ...
                .then((res) => {
                    ...
                    res.data.existencia.map((p) => {
                        if(p.tipo_operacion == 1){ p["operacion"] = "COMPRA"}else if(p.tipo_operacion == 2){p["operacion"] = "VENTA" }else if(p.tipo_operacion == 3){ p["operacion"] = "NOTA CREDITO"}else if(p.tipo_operacion == 4){ p["operacion"] = "VENT. ANULACION"};
                        p['precioM'] = '$ '+p.precio;
                        p['total_entradaM'] = '$ '+p.total_entrada
                        p['total_salidaM'] = '$ '+p.total_salida;
                        p['total_precioM'] = '$ '+p.total_precio;
                        if(p.stock_estado == 1){ p['estado'] = "Vigente"}else if(p.stock_estado == 0){p['estado'] = "N/A"}else if(p.stock_estado == 2){p['estado'] = "Caduco"}
