...
class ComprobanteController extends Controller
{
    ...
    {
        $datos = Comprobante::where('estado_id', 12)->get();
        $datos->load('UnidadNegocio', 'TipoComprobante');
        return $datos;
    }
    ...
    {
        $data = Comprobante::create(['codigo' => $this->GenerarCodigo(),'glosa' => $request->glosa, 'fecha_comprobante' => $request->fecha_comprobante, 'empresa_id' => $request->idEmpresa['id_empresa'],
                             'unidadnegocio_id' => $request->unidadnegocio['id_unidadnegocio'], 'tipocomprobante_id' => $request->comprobante['id_tipocomprobante'], 'estado_id' => 12]);
        return  ['mensaje' => 'Unidad de Negocio añadida exitosamente', 'codigo' => $data->codigo];
    }
    ...
    {   
        Comprobante::updateOrCreate(['id_comprobante' => $id],['estado_id' => 13]);
        return "Comprobante aprobado existosamente";
    }

    public function deleteComprobante($id)
    {   
        DetalleComprobante::where('comprobante_id', $id)->delete();
        Comprobante::destroy($id);

        return "Comprobante eliminado exitosamente.";
    }

    public function buscarComprobante(Request $request)
    {   
        $fecha = date('Y-m',strtotime($request->mes));
        $datos = Comprobante::where('estado_id', $request->estado['id'])->where('fecha_comprobante', 'LIKE','%'.$fecha.'%')->get();
        $datos->load('UnidadNegocio', 'TipoComprobante');
        return $datos;
    }
